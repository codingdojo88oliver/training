...
def dashboard(request):
	if 'is_logged_in' in request.session:
		if request.session['is_logged_in'] == True:
			...
			return render(request, "main/dashboard.html", {"user": user, "upcoming_events": events})
		else:
...
def deleteEvent(request):
	...
	event = Event.objects.get(id=request.POST['event_id'], user=user).delete()
	messages.success(request, "You just deleted an event!")
	return redirect('/dashboard')
