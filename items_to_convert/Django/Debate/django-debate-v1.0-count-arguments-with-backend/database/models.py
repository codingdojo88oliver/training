...
class ArgumentManager(models.Manager):
	def basic_validator(self, postData):
		...
		if 'argument' not in postData:
			errors["argument"] = "Argument is required"
		else:	
			if len(postData['argument']) < 10:
				errors["argument"] = "Argument should be at least 10 characters"
		if 'stance' not in postData:
			errors["stance"] = "Stance is required"
...
class Topic(models.Model):
	...
	def _get_leading_stance(self):
		leading_points = 0
		leading_argument = None
		for argument in self.topic_arguments.all():
			if argument.point is not None:
				if argument.point > leading_points:
					leading_points = argument.point
					leading_argument = argument

		if leading_argument is not None:
			return leading_argument.stance.capitalize() + " with " + str(leading_argument.point) + " points"
		else:
			""

	leading_stance = property(_get_leading_stance)

	def __unicode__(self):
		return self.leading_stance
