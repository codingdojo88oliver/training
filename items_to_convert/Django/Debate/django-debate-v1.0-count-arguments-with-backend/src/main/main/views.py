...
def upvote(request):
	try:
		user = User.objects.get(id=request.session['user_id'])
		argument = Argument.objects.get(id=request.POST['argument_id'])
	except Exception as e:
		messages.error(request, "User is not logged in")
		return redirect("/")

	if argument.user_id == request.session['user_id']:
		messages.error(request, "You are not allowed to upvote your own argument.")
		return redirect('/topics/' + request.POST['topic_id'])

	argument.point = F('point') + 1
	argument.save()
...
def downvote(request):
	...
	if argument.user_id == request.session['user_id']:
		messages.error(request, "You are not allowed to downvote your own argument.")
		return redirect('/topics/' + request.POST['topic_id'])
