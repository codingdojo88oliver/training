...
describe('(1 pt) When the user clicks ‘Move to Favorites’, the quote should no longer appear on the ‘Inspirational Quotes’ section.', function(){
    this.timeout(15000);
    before(function() {
    ...
    it("When the user clicks ‘Move to Favorites’, the quote should no longer appear on the ‘Inspirational Quotes’ section.", function(done){
                ...
                .end(function(err, res){
                    ...
                    .then(res => {
                        ...
                        .then(res => {
                            ...
                            .then(res => {
                                var $html = jQuery(res.text);
                                var inspirational_quotes = $html.find("#inspirational_quotes");
                                let redirect_url = res != undefined ? res.redirects[0] : "";
                                expect(redirect_url).to.have.string("/dashboard");
                                expect(inspirational_quotes.text()).to.not.have.string(quote.quote);
                                expect(inspirational_quotes.text()).to.not.have.string(quote.quoted_by);
                                done();
                            })
                            .catch(err =>{
                                return done(err);
