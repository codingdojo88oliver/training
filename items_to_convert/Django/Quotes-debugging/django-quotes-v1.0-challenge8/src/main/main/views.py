...
def removeFromFavorites(request):
		...
		favorite = Favorite.objects.get(id=request.POST['favorite_id']).delete()
		messages.success(request, "You just removed a quote from favorites!")
		return redirect('/dashboard')	

def deleteQuote(request):
		try:
			user = User.objects.get(id=request.session['user_id'])
			quote = Quote.objects.get(id=request.POST['quote_id'], user_id=request.session['user_id'])
		except Exception as e:
			messages.error(request, "User is not logged in")
			return redirect("/")

		quote = quote.delete()
		messages.success(request, "You just deleted a quote!")
		return redirect('/dashboard')
