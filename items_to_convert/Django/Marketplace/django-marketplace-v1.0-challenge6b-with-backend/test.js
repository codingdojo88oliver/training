...
describe('(1 pt) When the user clicks on the ‘Approve’ button, the product should be removed from the ‘Products for Sale’ list.', function(){
    before(function() {
    ...
    this.timeout(15000);
    it("When the user clicks on the ‘Approve’ button, the product should be removed from the ‘Products for Sale’ list.", function(done){
                                            ...
                                            .then(res => {
                                                ...
                                                .then(res => {
                                                    ...
                                                    .then(res => {
                                                        ...
                                                        .then(res => {
                                                            var $html = jQuery(res.text);
                                                            var price = ad2.price - 10;
                                                            var my_products = $html.find("#my-products").text();
                                                            expect(my_products).to.not.have.string(ad2.name);
                                                            expect(my_products).to.not.have.string('$'+price);
                                                            expect(my_products).to.not.have.string("Approve");      
                                                            done();
...
describe('(1 pt) When the seller clicks on the ‘Approve’ button, the product should reflect under the buyer’s ‘Items Bought’ section with the negotiated price.', function(){
    before(function() {
        request.get(base_url + '/logout')
        .end(function(err, res){
    ...
    this.timeout(15000);
    it("When the seller clicks on the ‘Approve’ button, the product should reflect under the buyer’s ‘Items Bought’ section with the negotiated price.", function(done){
        request.get(base_url)
        .end(function(err, res){
            var $html = jQuery(res.text);
            user2.csrfmiddlewaretoken = $html.find('input[name=csrfmiddlewaretoken]').val();
                                                        ...
                                                        .then(res => {
                                                            ...
                                                            .end(function(err, res){
                                                                ...
                                                                .then(res => {
                                                                    ...
                                                                    expect(bought_products).to.have.string(ad2.name);
                                                                    expect(bought_products).to.have.string('$'+price);   
                                                                    done();
                                                                })
                                                                .catch(err =>{
                                                                    return done(err);
...
describe('(1 pt) When other users log in to the Dashboard page, the product should no longer appear under ‘Products for Sale’ section.', function(){
    this.timeout(15000);
    it("When other users log in to the Dashboard page, the product should no longer appear under ‘Products for Sale’ section.", function(done){
        request.get(base_url)
        .end(function(err, res){
            var $html = jQuery(res.text);
            user5.csrfmiddlewaretoken = $html.find('input[name=csrfmiddlewaretoken]').val();
            ...
            .then(res => {
                ...
                var $html = jQuery(res.text);
                var price = ad2.price - 10;
                var my_products = $html.find("#bought-products").text();
