...
def dashboard():
	if 'is_logged_in' in session:
		if session['is_logged_in'] == True:
			...
			product_ids = []
			mysql = connectToMySQL()
			query = "SELECT * FROM products left join users on users.id = products.user  WHERE user = %(id)s;"
			data = {
				"id": session['user_id'],
			}
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM transactions left join products on products.id = transactions.product left join users on users.id = products.user WHERE buyer = %(id)s;"
			data = {
				"id": session['user_id']
			}
			...
			mysql = connectToMySQL()
			query = "SELECT products.category, products.status, products.name, products.price as product_price, users.first_name, users.last_name, negotiations.price, negotiations.negotiator,  negotiations.id FROM negotiations left join products on products.id = negotiations.product left join users on users.id = negotiations.negotiator WHERE negotiator = %(id)s || products.id IN %(product_ids)s;"
			data = {
				"id": session['user_id'],
...
def removeProduct():
	try:
		...
		mysql = connectToMySQL()
		query = "SELECT * FROM products WHERE id = %(product_id)s and user = %(id)s;"
		data = {
...
def negotiate():
	...
	mysql = connectToMySQL()
	query = "INSERT INTO negotiations (negotiator, product, price, created_at) VALUES (%(negotiator)s, %(product)s, %(price)s,  NOW());"
	data = {
		"negotiator": session['user_id'],
...
def approve():
	...
	mysql = connectToMySQL()
	query = "INSERT INTO transactions (product, buyer, price, created_at) VALUES (%(product)s, %(buyer)s, %(price)s, NOW());"
	data = {
		"buyer": negotiation[0]['negotiator'],
		 "product": negotiation[0]['product'],
	...
	data = {
			"status": "Sold",
			"product_id": negotiation[0]['product'],	
	}
	mysql.query_db(query, data)
