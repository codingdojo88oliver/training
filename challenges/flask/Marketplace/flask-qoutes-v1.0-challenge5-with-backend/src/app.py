...
def dashboard():
	if 'is_logged_in' in session:
		if session['is_logged_in'] == True:
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM transactions left join products on products.id = transactions.product WHERE buyer = %(id)s;"
			data = {
				"id": session['user_id']
			}
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM products left join users on users.id = products.user  WHERE status = %(status)s;"
			data = {
				"status": "Available",
			}
