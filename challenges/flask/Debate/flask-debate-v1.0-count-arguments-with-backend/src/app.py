...
def dashboard():
	if 'is_logged_in' in session:
		if session['is_logged_in'] == True:
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM topics  left join users on users.id = topics.user_id left join arguments on arguments.user_id = users.id WHERE topics.user_id = %(id)s;"
			data = {
				"id": session['user_id']
			}
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM topics left join users on users.id = topics.user_id left join arguments on arguments.user_id = users.id WHERE  topics.user_id != %(id)s ORDER BY point desc;"
			data = {
				"id": session['user_id']
			}
...
def createTopic():
	...
	if request.method == "POST":
		...
		if len(errors) > 0:
			...
			mysql = connectToMySQL()
			query = "INSERT INTO topics (user_id, title, description,created_at) VALUES (%(user_id)s, %(title)s, %(description)s,NOW());"
			data = {
			...
			flash("You just created a topic!")
			return redirect('/dashboard')
...
def createArgument():
	...
	if request.method == "POST":
		...
		if len(errors) > 0:
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM users left join topics on topics.id = %(topic_id)s WHERE topics.user_id = %(user_id)s;"
			data = {
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM arguments left join users on users.id = arguments.user_id left join topics on topics.id = arguments.topic_id WHERE arguments.topic_id = %(topic_id)s;"
			data = {
				"topic_id": request.form['topic_id']
			}
			...
			mysql = connectToMySQL()
			query = "INSERT INTO arguments (user_id, topic_id, stance, argument, created_at) VALUES (%(user_id)s, %(topic_id)s, %(stance)s, %(argument)s, NOW());"
			data = {
				"user_id": session['user_id'],
				"topic_id": request.form['topic_id'],
...
def showTopic(id):
	...
	try:
		mysql = connectToMySQL()
		query = "SELECT * FROM topics left join users on users.id = topics.user_id WHERE topics.id = %(id)s;"
		data = {
			"id": {id}
		}
		topic = mysql.query_db(query, data)
	...
	mysql = connectToMySQL()
	query = "SELECT * FROM arguments left join topics on topics.id = arguments.topic_id  left join users on users.id = topics.user_id Order BY point;"
...
def upvote():
	...
	if argument[0]['user_id'] == session['user_id']:
