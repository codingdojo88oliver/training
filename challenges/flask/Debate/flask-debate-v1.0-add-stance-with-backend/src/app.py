...
def dashboard():
	if 'is_logged_in' in session:
		if session['is_logged_in'] == True:
			...
			mysql = connectToMySQL()
			query = "SELECT * FROM topics left join users on users.id = topics.user_id WHERE user_id = %(id)s;"
			data = {
				"id": session['user_id']
			}
...
def createTopic():
	...
	if request.method == "POST":
		...
		if len(errors) > 0:
			...
			mysql = connectToMySQL()
			query = "INSERT INTO topics (title, description, user_id,created_at) VALUES (%(title)s, %(description)s, %(users_id)s,NOW());"
			data = {
...
def createArgument():
	...
	if request.method == "POST":
		...
		if len(errors) > 0:
			...
			Argument = mysql.query_db(query, data)
			print(Argument)
			flash("You said: " + request.form['stance'] + " on this topic")
			return redirect('/topics/' + request.form['topic_id'])
...
def showTopic(id):
	...
	mysql = connectToMySQL()
	query = "SELECT * FROM arguments left join topics on topics.id = arguments.topic_id  left join users on users.id = topics.user_id Order BY ;"
	arguments = mysql.query_db(query, data)
	
	return render_template("topic.html", topic = topic, arguments = arguments)
