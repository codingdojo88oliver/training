...
def dashboard():
	if 'is_logged_in' in session:
		if session['is_logged_in'] == True:
			...
			mysql = connectToMySQL()
			query = "SELECT users.name as user_name, events.id, events.name, events.date FROM events left join users on users.id = events.user_id WHERE  events.user_id = %(id)s;"
			...
			mysql = connectToMySQL()
			events = "SELECT users.name as user_name, events.id, events.name, events.date FROM events left join users ON  users.id = events.user_id WHERE events.id NOT IN %(id_s)s"
			data = {
				"id_s": events_im_hosting_ids
			}
			upcoming_events = mysql.query_db(events,data)

			return render_template("dashboard.html",user = user, upcoming_events = upcoming_events, events_im_hosting = events_im_hosting)
		else:
