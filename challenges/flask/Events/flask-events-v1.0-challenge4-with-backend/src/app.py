...
def dashboard():
	if 'is_logged_in' in session:
		if session['is_logged_in'] == True:
			...
			mysql = connectToMySQL()
			query = "SELECT users.name as user_name, events.name, events.date, events.id FROM events left join users on users.id = events.user_id WHERE  events.user_id = %(id)s;"
			...
			mysql = connectToMySQL()
			events = "SELECT users.name as user_name, events.name, events.date, events.id FROM events left join users ON  users.id = events.user_id WHERE events.id NOT IN %(id_s)s "
			data = {
