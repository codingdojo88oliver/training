                                  ...
                                  if (products.length > 0) {
                                      ...
                                      connection.query("SELECT *, products.id, users.first_name as user_first_name, users.last_name as user_last_name  FROM products \
                                            ...
                                            connection.query("SELECT *, products.name as product_name, products.user_id, users.first_name as product_user_first_name, users.last_name as product_user_last_name FROM transactions \
                                                    ...
                                                    connection.query("SELECT *, products.name as product_name, products.price as product_price, users.first_name as user_first_name, users.last_name as user_last_name, negotiations.id,  negotiations.price, negotiations.negotiator_id  FROM negotiations \
...
app.post('/approve', function(req, res) {
     connection.query("SELECT *FROM users WHERE id = ?", [req.session.user_id], (err, user) =>{
         connection.query("SELECT *FROM negotiations WHERE id = ?", [req.body.negotiation_id], (err, negotiation) =>{
             if (err) {
                 req.send('messages', "You are not allowed to approve this negotiation");
             }
             else{
                    connection.query("INSERT INTO transactions (product_id, buyer_id, price) VALUES(?,?,?)",
                            [negotiation[0].product_id, negotiation[0].negotiator_id, negotiation[0].price], (err, transaction) =>{

                                var product_status = "Sold";
                                connection.query("UPDATE products SET status = ? WHERE id = ?", [product_status, negotiation[0].product_id], (err, products) =>{
                                    req.flash('messages', "Asking price approved!");
                                    res.redirect('/dashboard');                                                
                                });
                    });
