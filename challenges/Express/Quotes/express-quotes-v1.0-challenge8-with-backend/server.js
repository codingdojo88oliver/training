...
app.post('/delete-quote', function(req, res) {
        connection.query("SELECT * FROM users WHERE id = ?", [req.session.user_id], (err, user) =>{
                if (err) {
                    req.flash('messages', "User is not logged in")
                    res.redirect("/");
                    ...
                    connection.query("DELETE FROM quotes WHERE id = ?", [req.body.quote_id], (err, quote) =>{
                        req.flash('messages', "You just deleted a quote!")
                        res.redirect('/dashboard')    
                    });
                }
        });
