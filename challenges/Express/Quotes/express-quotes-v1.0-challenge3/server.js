            ...
            connection.query("SELECT *FROM users WHERE id = ?", [req.session.user_id], (err, user) =>{
                if (err) {
                    ...
                    res.redirect("/")
                     ...
                     connection.query("SELECT *, users.name as user_name FROM quotes\
                             LEFT JOIN users ON users.id = quotes.user_id", (err, quotes) =>{
...
app.post('/create-quote', function(req, res) {
    ...
    if (errors.length) {
        ...
        connection.query("INSERT INTO quotes (user_id, quoted_by, quote) VALUES(?,?,?)",
                       [req.session.user_id, req.body.quoted_by, req.body.quote], (err, quote) =>{
        });
        res.redirect('/dashboard');
    }
    else{
