...
app.post('/remove-from-favorites', function(req, res) {
        connection.query("SELECT * FROM users WHERE id = ?", [req.session.user_id], (err, user) =>{
                if (err) {
                    ...
                    connection.query("SELECT * FROM favorites WHERE id = ?", [req.body.favorite_id], (err, favorite) =>{
                          
                           connection.query("DELETE FROM favorites WHERE user_id = ? AND quote_id = ?", [favorite[0].user_id, favorite[0].quote_id], (err, favorites) =>{
