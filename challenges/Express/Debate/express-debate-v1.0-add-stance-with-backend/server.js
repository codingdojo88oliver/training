        ...
        connection.query("SELECT *, users.name as user_name, topics.id FROM topics \
                ...
                if (err) {
                    ...
                    res.redirect('/dashboard');
                     ...
                     connection.query("SELECT *,users.name as user_name, arguments.user_id FROM arguments\
                         LEFT JOIN topics ON topics.id = arguments.topic_id \
                         LEFT JOIN users ON users.id = topics.user_id ", (err, arguments) =>{
                          res.render('topic', { topic:topic, arguments: arguments})
                     });
                 }
        }); 
});
