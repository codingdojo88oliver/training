...
app.get('/planes/:id', function(req, res) {
      connection.query("SELECT *FROM users WHERE users.id = ?", [req.session.user_id], (err, user) =>{
                connection.query("SELECT *FROM planes WHERE user_id = ? AND id = ?", [user[0].id, req.params.id], (err, plane) =>{
                           if (err) {
                                res.send("You are not allowed to view this message")
                           }
                           else{
                                res.render("show-my-plane", {plane: plane})
                           }
...
app.post('/delete-plane', function(req, res) {
       connection.query("SELECT *FROM users WHERE users.id = ?", [req.session.user_id], (err, user) =>{
                connection.query("SELECT *FROM planes WHERE user_id = ? AND id = ?", [user[0].id, req.body.plane_id], (err, plane) =>{
                           if (err) {
                                    req.flash('messages', "You are not allowed to delete this virtual paper plane");
                                    res.redirect("/dashboard");
                           }
                           else{
                                    connection.query("DELETE FROM messages WHERE plane_id = ?", [plane[0].id], (err, Message) =>{
                                        connection.query("DELETE FROM planes WHERE id = ?", [plane[0].id], (err, Plane) =>{
                                            req.flash('messages', "Plane successfully deleted");
                                            res.redirect("/dashboard");
                                        });
                                    });
                           }
