...
describe('(1 pt) When the user clicks the ‘Leave’ button, the page is refreshed and the specific event should no longer be displayed under the ‘Events Im Joining’ section.', function(){
    it("When the user clicks the ‘Leave’ button, the page is refreshed and the specific event should no longer be displayed under the ‘Events Im Joining’ section.", function(done){
        request.get(base_url)
        .end(function(err, res){
            var $html = jQuery(res.text);
            user1a.csrfmiddlewaretoken = $html.find('input[name=csrfmiddlewaretoken]').val();
